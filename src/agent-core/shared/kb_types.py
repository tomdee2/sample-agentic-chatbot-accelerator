...
#
# Knowledge Base types shared by both docker and docker-swarm agent implementations.
# --------------------------------------------------------------------------------- #
...
class BedrockRerankingConfiguration(BaseModel):
    ...
    Attributes:
        ...
        numberOfRerankedResults (Optional[int]): Alternative field name for number of results (swarm compatibility).
    """
    ...
    numberOfRerankedResults: Optional[int] = None  # Alternative field name for swarm
...
class ReferenceLocation(BaseModel):
    ...
    Attributes:
        ...
        webLocation (Optional[dict]): Location details for web references
    ...
    def get_id(self) -> str:
        ...
        Raises:
            ValueError: If the location type is not supported, or if the URI is invalid.
        ...
        if self.type == ELocationType.S3 and self.s3Location:
            ...
            ref_id = self.webLocation.get("url", "null")
